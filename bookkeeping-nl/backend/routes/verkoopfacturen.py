...
def lijst():
    ...
    return jsonify(
        db.get_verkoopfacturen(current_app.config["DB_PATH"], status)
    )
...
def aanmaken():
    ...
    validate_sales_tax_profile(
        ...
        bool(data.get("omgekeerde_heffing")),
    )

    # Auto-generate invoice number if not provided
    if not data.get("factuurnummer"):
        instellingen = db.get_all_instellingen(db_path)
        data["factuurnummer"] = next_factuurnummer(db_path, instellingen)

    # Calculate VAT on each rule
    calculated_regels = []
    for regel in regels:
        calculated_regels.append(
...
def update_status(fid):
    ...
    if db.get_verkoopfactuur(current_app.config["DB_PATH"], fid) is None:
        return jsonify({"error": "Niet gevonden"}), 404
